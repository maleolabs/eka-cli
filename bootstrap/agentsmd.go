...
func buildAgentsMDBlock(project, namespace string) []byte {
	...
	b.WriteString("This is an EKA repository: engineering knowledge lives in the canonical\n")
	b.WriteString("store, not in repo files. Work the loop: Understand → Context → Reason\n")
	b.WriteString("→ Change → Validate → Publish.\n")
	b.WriteString("Read: `eka status` (position), `eka get <ns>/<type>:<id>` (one object),\n")
	b.WriteString("  `eka context <subject>` (constraints, `--depth engineering` for full),\n")
	b.WriteString("  `eka view <projection>` (human board, never a retrieval contract).\n")
	b.WriteString("Change: `eka new` (draft) → `eka validate` → `eka publish`; state via\n")
	b.WriteString("  `eka transition <line> <to>`; evidence via `eka note --role <role>`;\n")
	b.WriteString("  ownership via `eka assign <item> --to <mbr>`.\n")
	b.WriteString("Rules: never edit store data directly; transitions are gated and\n")
	b.WriteString("  forward-only (read actual state via `eka get` first); after each\n")
	b.WriteString("  transition run `eka sync push` (never full `eka sync` mid-execution);\n")
	b.WriteString("  identity form is `<ns>/<type>:<id>`.\n")
	b.WriteString("Skills: route via `eka-router` first; directly: `eka-orientation`\n")
	b.WriteString("  (mental model), `eka-engineering-workflow` (execution loop),\n")
	b.WriteString("  `eka-troubleshooting` (refusals/errors).\n")
